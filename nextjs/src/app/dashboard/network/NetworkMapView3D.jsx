...
export default function NetworkMapView3D({ nodes = [], edges = [], selectedNodeId, onNodeClick, height = 580, is3D = true }) {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        console.error('[NetworkMapView3D] init error:', err);
  ...
  if (error) {
    return (
      <div style={{ height, display: 'flex', alignItems: 'center', justifyContent: 'center', background: '#f8fafc', borderRadius: 16, border: '1px solid #e2e8f0', fontFamily: 'monospace', fontSize: 12, color: '#94a3b8' }}>
        ⚠ Map failed: {error}
      </div>
    );
  }

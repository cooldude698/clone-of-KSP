...
const ANPR_NODES = [
  ...
  { id: 'CAM-ATT-0001', name: 'Attibele Inter-State Border',    lat: 12.7800, lng: 77.7200, status: 'INTERCEPT_READY' },
];
...
export default function NetworkMapView({ nodes, edges, selectedNodeId, onNodeClick, height = 580 }) {
  const containerRef  = useRef(null);
  const mapRef        = useRef(null);
  const layerGroupRef = useRef(null);

  const tileUrl = process.env.NEXT_PUBLIC_MAPS_TILE || 'https://{s}.tile.openstreetmap.org/{z}/{x}/{y}.png';

  // ── Init map (pure dynamic import) ────────────────────────────────────────
  useEffect(() => {
    ...
    import('leaflet').then((mod) => {
      ...
      const map = L.map(containerRef.current, {
        center: [12.9716, 77.6000], zoom: 11,
        scrollWheelZoom: true, zoomControl: true,
      });

      L.tileLayer(tileUrl, {
        attribution: '&copy; OpenStreetMap contributors • KSP Telemetry', maxZoom: 18,
      }).addTo(map);
  ...
  useEffect(() => {
    ...
    import('leaflet').then((mod) => {
      ...
      // Predictive route polylines
      PREDICTIVE_ROUTES.forEach((route) => {
        const latlngs = route.waypoints.map(w => [w.lat, w.lng]);

        L.polyline(latlngs, { color: route.color, weight: 6, opacity: 0.25, lineCap: 'round' }).addTo(layerGroup);
        const polyline = L.polyline(latlngs, {
          color: route.color, weight: 2.5, opacity: 0.9, dashArray: '8, 6',
        }).addTo(layerGroup);
        ...
        route.waypoints.forEach((wp) => {
          const isStart = wp.type === 'start', isIntercept = wp.type === 'intercept', isDest = wp.type === 'destination';
          const color  = isStart ? '#DC2626' : isIntercept ? '#F59E0B' : isDest ? '#8B5CF6' : route.color;
          const radius = isStart ? 8 : isIntercept ? 7 : 5;
          const circle = L.circleMarker([wp.lat, wp.lng], {
            radius, fillColor: color, color: '#FFFFFF', weight: 2, fillOpacity: 0.95,
          }).addTo(layerGroup);
          circle.bindTooltip(`
            <div style="font-family:sans-serif;padding:4px 8px;min-width:160px;">
              ...
              <div style="font-size:9.5px;color:${color};font-weight:bold;margin-top:3px;font-family:monospace;">Multi-Source Intel Verified</div>
            </div>
      ...
      // ANPR cameras
      ANPR_NODES.forEach((cam) => {
        L.circle([cam.lat, cam.lng], {
          radius: 1200, color: '#06B6D4', weight: 1, dashArray: '4, 4',
          fillColor: '#06B6D4', fillOpacity: 0.08,
        }).addTo(layerGroup);
        const camMarker = L.circleMarker([cam.lat, cam.lng], {
          radius: 6, fillColor: '#06B6D4', color: '#FFFFFF', weight: 1.5, fillOpacity: 1,
        }).addTo(layerGroup);
      ...
      (nodes || []).forEach((node) => {
        if (node.district === 'Bengaluru Urban' || node.id === 'SUS-8842') {
          const pin = L.circleMarker([12.9175, 77.6215], {
            radius: 11, fillColor: '#DC2626', color: '#FFFFFF', weight: 2.5, fillOpacity: 0.95,
          }).addTo(layerGroup);
          pin.bindTooltip(`
          ...
          pin.on('click', () => { if (onNodeClick) onNodeClick(node.id); });
        }
      });
      ...
      if (!mapRef.current._drishtiLegend) {
        ...
        legend.onAdd = function () {
          ...
          div.style.cssText = `background:rgba(15,23,42,0.92);border:1px solid rgba(51,65,85,0.8);
            border-radius:14px;padding:12px 14px;font-family:monospace;font-size:11px;
            color:#F8FAFC;backdrop-filter:blur(10px);line-height:1.8;min-width:240px;
            box-shadow:0 15px 30px rgba(0,0,0,0.3);`;
          div.innerHTML = `
            <div style="font-weight:700;font-size:10px;letter-spacing:0.08em;color:#94A3B8;margin-bottom:6px;text-transform:uppercase;border-bottom:1px solid rgba(51,65,85,0.6);padding-bottom:3px;">CIA-Calibrated Intercept Grid</div>
            <div style="display:flex;align-items:center;gap:8px;"><span style="display:inline-block;width:10px;height:10px;border-radius:50%;background:#DC2626;flex-shrink:0;"></span><span><b>Target Sighting</b> (Last Confirmed)</span></div>
            <div style="display:flex;align-items:center;gap:8px;"><span style="display:inline-block;width:18px;height:2px;background:#3B82F6;border-top:2px dashed #3B82F6;flex-shrink:0;"></span><span style="color:#60A5FA;">Predicted Escape Corridor</span></div>
            <div style="display:flex;align-items:center;gap:8px;"><span style="display:inline-block;width:10px;height:10px;border-radius:50%;background:#F59E0B;flex-shrink:0;"></span><span style="color:#FBBF24;">Active Intercept Chokepoint</span></div>
            <div style="display:flex;align-items:center;gap:8px;"><span style="display:inline-block;width:10px;height:10px;border-radius:50%;background:#06B6D4;flex-shrink:0;"></span><span style="color:#22D3EE;">ANPR Surveillance Sensor</span></div>
          `;

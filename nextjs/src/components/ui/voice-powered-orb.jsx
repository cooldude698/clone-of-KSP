...
/**
 * VoicePoweredOrb — WebGL shader orb that reacts to voice/microphone input.
 *
 * Props:
 *  className           — additional CSS classes for the container div
 *  hue                 — hue rotation in degrees (0–360)
 *  enableVoiceControl  — whether to activate mic and react to voice
 *  voiceSensitivity    — multiplier for RMS voice level (default 1.5)
 *  maxRotationSpeed    — max extra rotation speed when voice is loud (default 1.2)
 *  maxHoverIntensity   — max hover/wobble distortion from voice (default 0.8)
 *  onVoiceDetected     — callback(bool) fired when voice detection changes
 */
export function VoicePoweredOrb({
  ...
  const stopMicrophone = () => {
    try {
      ...
      console.warn('[VoicePoweredOrb] stopMicrophone error:', err);
    }
  };
  ...
  const initMicrophone = async () => {
    try {
      ...
      console.warn('[VoicePoweredOrb] Microphone init failed:', err);
      return false;
    }
  };
  ...
  useEffect(() => {
    ...
    try {
      ...
      if (enableVoiceControl) {
        initMicrophone().then(ok => { isMicReady = ok; });
      }
      ...
      const update = (t) => {
        ...
        if (enableVoiceControl && isMicReady) {
          ...
          const rotSpeed = baseRotSpeed + voiceLevel * maxRotationSpeed * 2.0;
          if (voiceLevel > 0.05) currentRot += dt * rotSpeed;
  ...
  // Handle mic state changes when enableVoiceControl toggles
  useEffect(() => {
    let mounted = true;
    if (enableVoiceControl) {

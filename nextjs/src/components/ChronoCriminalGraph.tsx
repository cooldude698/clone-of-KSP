    ...
    const nodeEnter = nodeSel.enter()
      ...
      .on("mouseover", (event, d) => {
        ...
        tooltipRef.current.innerHTML = `
          <div class="font-bold text-paper-100 text-sm mb-1">${d.label}</div>
          <div class="text-paper-100/70 text-xs">Total Cases: <span class="text-phosphor-500 font-semibold">${d.total_firs}</span></div>
          <div class="text-paper-100/70 text-xs">Risk Score: <span class="font-bold" style="color: ${d.risk_score > 70 ? 'var(--color-critical-500)' : d.risk_score > 40 ? 'var(--color-warn-500)' : 'var(--color-phosphor-500)'}">${d.risk_score}</span></div>
  ...
  return (
    <div ref={containerRef} className="relative w-full rounded-2xl bg-steel-700 p-6 border border-steel-600/50 shadow-2xl flex flex-col select-none overflow-hidden text-paper-100 transition-colors duration-200">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-4 z-10">
        <div>
          <h3 className="text-lg font-bold text-paper-100 tracking-wide font-mono">
        ...
        {/* Playback Date Badge */}
        <div className="px-4 py-2 rounded-xl bg-void-000 border border-steel-600/50 flex items-center gap-3">
          <div className="w-2 h-2 rounded-full bg-phosphor-500 animate-pulse" />
          <span className="text-sm font-semibold font-mono text-phosphor-500">
            {currentDateString}
          </span>
        </div>
      </div>
      ...
      <div
        ref={tooltipRef}
        className="pointer-events-none absolute left-0 top-0 z-50 rounded-xl border border-steel-600 bg-steel-700/95 p-3 shadow-xl backdrop-blur-md opacity-0 transition-opacity duration-200 w-52 text-paper-100"
        style={{ transform: "translate(0px, 0px)" }}
      />

      {/* Visualization Canvas */}
      <div className="relative w-full rounded-xl bg-void-000/30 border border-steel-600/50 overflow-hidden flex-grow" style={{ height }}>
        <svg
          ref={svgRef}
      ...
      <div className="mt-6 flex flex-col sm:flex-row items-center gap-4 bg-void-000/40 border border-steel-600/40 p-4 rounded-xl">
